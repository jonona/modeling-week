...
# -*- coding: utf-8 -*-
"""
Created on Wed Aug 19 09:16:33 2020
...
#X = np.expand_dims(f_eigval,axis=1)
...
# X = Tensors.reshape((22,9))
# X = np.concatenate((X, np.expand_dims(f_eigval, axis=1)), axis=1)
...
test_matrix = np.stack((y_test,reg.predict(X_test),angle_eigval[idx_test]), axis=1)
train_matrix = np.stack((y_train,reg.predict(X_train),angle_eigval[idx_train]), axis=1)
...
print("\n\n==================== COEFFICIENTS ==================")
# df = pd.DataFrame({'Parameter': ['a11', 'a12', 'a13', 'a21', 'a22', 'a23', 'a31', 'a32', 'a33', 'f_eigval'], 'Coefficients': getattr(reg, 'coef_')})
# df = pd.DataFrame({'Parameter': ['f_eigval'], 'Coefficients': getattr(reg, 'coef_')})
# print(df)
